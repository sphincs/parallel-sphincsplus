...
static bool test( const unsigned char *expected_result,
    ...
    for (unsigned n = 1; n <= len_message; n++) {
        ...
        for (int track = 0; track < 4; track++) {
            ...
            const unsigned char *b0 = (track==0) ? message : dummy_buf+0;
            const unsigned char *b1 = (track==1) ? message : dummy_buf+0;
            const unsigned char *b2 = (track==2) ? message : dummy_buf+0;
            const unsigned char *b3 = (track==3) ? message : dummy_buf+0;
...
bool test_sha512(bool fast_flag, enum noise_level level) {
    (void)fast_flag;
    (void)level;
    ...
    // The below test vectors were extracted from NIST published values
    {
        ...
        if (!test( output, message, sizeof message )) return false;
    }
    {
        ...
        if (!test( output, message, sizeof message - 1 )) return false;
    }
    {
