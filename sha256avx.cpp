                     ...
                     unsigned long long len) 
{
    ...
    const unsigned char *d4 = static_cast<const unsigned char *>(i4);
    const unsigned char *d5 = static_cast<const unsigned char *>(i5);
    const unsigned char *d6 = static_cast<const unsigned char *>(i6);
    const unsigned char *d7 = static_cast<const unsigned char *>(i7);

    int datalen = ctx->datalen;
    while(i < len) {
        int bytes_to_copy = len - i;
	if (bytes_to_copy + datalen > 64) bytes_to_copy = 64 - datalen;
        memcpy(&ctx->msgblocks[64*0+datalen], d0 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*1+datalen], d1 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*2+datalen], d2 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*3+datalen], d3 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*4+datalen], d4 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*5+datalen], d5 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*6+datalen], d6 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*7+datalen], d7 + i, bytes_to_copy);
