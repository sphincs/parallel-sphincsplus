 ...
 */
void key_sha256_L35_robust::thash( unsigned char *out,
    ...
    mgf1<SHA512_CTX> bitstream( mgf1_seed, n + sha256_addr_bytes );
 ...
 */
void key_sha256_L35_robust::thash_xn(unsigned char **out,
             unsigned char **in,
             ...
             addr_t* addrx8)
{
    ...
    unsigned char seed[ 4*(max_len_hash + sha256_addr_bytes) ];
    for (int j=0; j<4; j++) {
	mask_seed[j] = &seed[ j * (max_len_hash+sha256_addr_bytes) ];
    }
    ...
    for (int i=0; i<8; i+=4) {
	    ...
	    memcpy( mask_seed[j], get_public_seed(), n );
            memcpy( mask_seed[j]+n, &addrx8[i+j], sha256_addr_bytes );
	}
        mgf1_sha512_4x bit_stream( mask_seed, n + sha256_addr_bytes );
...
// uses SHA-256
void key_sha256_L35_robust::f_xn(unsigned char **out, unsigned char **in,
                                 addr_t* addrxn) {
    key_sha256_robust::thash_xn(out, in, 1, addrxn);
}

// This precomputes the SHA-512 hash state after processing the public seed
void key_sha256_L35_robust::initialize_public_seed(const unsigned char *pub_seed) {
